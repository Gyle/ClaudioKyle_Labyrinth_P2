...
public class Door : MonoBehaviour
{
    ...
    public GameObject[] activateTargets; //if using a different gameobject as the activation target place it here, this is helpful for centering a hinge gate's activationrange
       ...
       // print(state + "state");
        ...
        if (distanceTrigger == true)
        {
            if (state == 0)
            {
                for(int i = 0; i < activateTargets.Length; i++){
                    ...
                    {
                        if (Vector3.Distance(activateTargets[i].transform.position, player.transform.position) <= openRange)
                        {
                            if (openSounds.Length > 0)
                            {
        ...
        if (state == 2)
        {
            ...
            for(int i = 0; i < activateTargets.Length; i++){
                ...
                if (activateTargets[i] != null)
                {
                    if (Vector3.Distance(activateTargets[i].transform.position, player.transform.position) >= closeRange)
                    {
                        if (closeSounds.Length > 0)
                        {
    ...
    void OnDrawGizmos()
    {
            ...
            if (showOpenRange == true)
        {
            ...
            for(int i = 0; i < activateTargets.Length; i++){
                if (activateTargets[i] != null)
                {
                    Gizmos.DrawSphere(activateTargets[i].transform.position, openRange);
                }
                else
                {
        ...
        if (showCloseRange == true)
        {
            ...
            for(int i = 0; i < activateTargets.Length; i++){
                if (activateTargets[i] != null)
                {
                    Gizmos.DrawSphere(activateTargets[i].transform.position, closeRange);
                }
                else
                {
